...
| `JWT_ACCESS_EXPIRES` | `900` | (опционально) Время жизни access-токена в секундах (15 мин) |
| `JWT_REFRESH_EXPIRES` | `2592000` | (опционально) Время жизни refresh-токена (30 дней) |

### Хранилище файлов (аватары, голосовые, фото, видео)

**⚠️ Обязательно для загрузки файлов:** Railway использует read-only файловую систему. Без Volume загрузка файлов не работает (ошибка "Failed to save file").

**Вариант A: Локальное хранилище (Volume на Railway)**

1. В Railway: **ваш backend-сервис** → **Settings** → **Volumes** → **Add Volume**
2. Mount path: **`/app/uploads`** (должно совпадать точно)
...
Entrypoint в Dockerfile автоматически выдаёт права на запись в Volume для пользователя приложения.

**Вариант B: Cloudflare R2** (требует доработки контроллера — сейчас используется локальное хранилище)
...
### Ошибка 500 при загрузке аватара/файлов

- **Локальное хранилище:** создайте Volume с mount path `/app/uploads` и задайте `FILE_UPLOAD_DIR=/app/uploads`
- Проверьте логи Railway — теперь выводится точная причина (IOException, права доступа и т.п.)
